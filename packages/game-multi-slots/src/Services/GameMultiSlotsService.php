...
namespace Packages\GameMultiSlots\Services;
...
class GameMultiSlotsService extends GameService
{
    protected $gameableModel = GameMultiSlots::class;
    ...
    public function __construct($user = NULL)
    {
        ...
        // randomly choose slots game index when it is created from console and HTTP request is not available
        $this->index = request()->route()->index ?? random_int(0, count(config('game-multi-slots.titles'))-1);

        $this->slot = new Slot(array_map(function($v) {
            return count($v);
        }, config('game-multi-slots.reels')[$this->index]));
    }

    protected function createGameable(): Model
    {
        $gameable = new GameMultiSlots();
    ...
    public function play($params): GameService
    {
        ...
        $syms = config('game-multi-slots.symbols')[$this->index];
        $reels = config('game-multi-slots.reels')[$this->index];

        $this->game->gameable->lines_bet      = $params['lines_count'];
        $this->game->gameable->bet_amount     = $params['bet'];
        ...
        if ($this->game->gameable->scatters_count) {
            $idx_scatter = -1;
            foreach ($syms as $idx => $sym) if ($sym['scatter']) {
                $idx_scatter = $idx;
                break;
            }

            if ($syms[$idx_scatter]['w' . ($this->game->gameable->scatters_count > 5 ? 5 : $this->game->gameable->scatters_count) . 't'] == 'x')
                $win_scatters_ttl = $this->game->gameable->bet_amount * $this->game->gameable->lines_bet * $syms[$idx_scatter]['w' . ($this->game->gameable->scatters_count > 5 ? 5 : $this->game->gameable->scatters_count)];
            else
                $win_scatters_ttl = $syms[$idx_scatter]['w' . ($this->game->gameable->scatters_count > 5 ? 5 : $this->game->gameable->scatters_count)];
        ...
        foreach (GameMultiSlotsService::lines as $line_idx => $line) {
            ...
            for ($reel = 1; $reel < 6; $reel++) {
                ...
                if (
                    ...
                    if ($syms[$sym_idx]['wild'])
                        $sym_cnt = 1 + $wild_cnt;
                    elseif ($syms[$sym_idx]['w' . $sym_cnt] && !$syms[$sym_idx]['scatter']) {
                        $win_line_new = 0;
                        $win_pos_new = [];
