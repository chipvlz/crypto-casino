				...
				<div class="section-paytable show" data-section="paytable">
					@foreach ($symbols as $sym)
					    <div class="paytable-symbol">
							<div class="image"><img src="{{ asset('storage/games/multi-slots/'.$index.'/'.$sym['filename']) }}"></div>
							<div class="lines">
								@if($sym["wild"])
						...
						@foreach ($reels as $reel)
							<div class="reel">
								@foreach ($reel as $sym_idx)
									<div class="image"><img src="{{ asset('storage/games/multi-slots/'.$index.'/'.$symbols[$sym_idx]['filename']) }}"></div>
								@endforeach
							</div>
						@endforeach
					</div>
	...
	<script>
		window.addEventListener("DOMContentLoaded",function(){
			...
			window.game_slots_proto({
				...
				min_bet					: {{ config('game-multi-slots.min_bet')[$index] }},
				max_bet					: {{ config('game-multi-slots.max_bet')[$index] }},
				max_bet					: {{ config('game-multi-slots.max_bet')[$index] }},
				bet_change_amount		: {{ config('game-multi-slots.bet_change_amount')[$index] }},
				default_bet				: {{ config('game-multi-slots.default_bet')[$index] }},
				default_lines			: {{ config('game-multi-slots.default_lines')[$index] }},
				balance					: {{ auth()->user()->account->balance }},
				reels					: {{ json_encode($reels) }},
				lines					: {{ json_encode(\Packages\GameMultiSlots\Services\GameMultiSlotsService::lines) }},
				animation_frames		: 14,
				animation_time			: 28,
				animation_size			: 200
			});
