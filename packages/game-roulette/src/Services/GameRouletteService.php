...
namespace Packages\GameRoulette\Services;

use App\Helpers\Games\Roulette;
use App\Services\GameService;
use Illuminate\Database\Eloquent\Model;
use Packages\GameRoulette\Models\GameRoulette;

class GameRouletteService extends GameService
{
    const BET_TYPES = [
    ...
    protected $gameableModel = GameRoulette::class;
    ...
    private function calcWinAmount($bets, $winNumber): int
    {
        ...
        foreach ($bets as $type => $bet) {
            if (in_array($winNumber, self::BET_TYPES[$type])) {
                $amount += $bet * (36 / count(self::BET_TYPES[$type]));
            }
        }
