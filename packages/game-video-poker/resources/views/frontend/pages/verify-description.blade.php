<p>
    ...
    {{ __('The server secret represents randomly shuffled card deck.') }}
    {{ __('The server seed is a cryptographically secure random alpha-numeric string.') }}
    {{ __('The hash of these 2 strings helps to ensure that the initial card deck is not altered after you choose the bet amount.') }}
...
<p>
    ...
    {{ __('The result number will be divided by 52 (number of cards in the deck) and N cards will be cut (moved from the top to the bottom of the deck), where N is the remainder of the division.') }}
    {{ __('Because the client seed can not be predicted by the server the number of cards cut from the initial deck is completely random and hence you can be sure that the game result is fair.') }}
</p>